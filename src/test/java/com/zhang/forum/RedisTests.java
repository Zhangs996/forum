...
public class RedisTests {
    ...
    // 多次访问同一个key，开销很大。怎么解决？把key绑定在一个对象。
    @Test
    public void testBoundOperations() {
        String redisKey = "test:count";
//        也可以绑定成BoundHashOperations
    ...
    // redis支持的事务不完全支持ACID，redis管理的事务是比较简单的。
//    它的逻辑是：当执行一个redis命令的时候，不会立刻执行，而是把命令放进队列里先存着，直到一个事务提交时所有的命令都提交。
//    尽量别查询，因为会延迟
    ...
    public void testTransactional() {
        Object obj = redisTemplate.execute(new SessionCallback() {
            ...
            public Object execute(RedisOperations operations) throws DataAccessException {
                ...
                System.out.println(operations.opsForSet().members(redisKey));//事务未提交，查不出来，此时查询是没有效果的

                return operations.exec();//提交事务
            }
        });
        System.out.println(obj);//能打印出来，事务已经提交
    }
